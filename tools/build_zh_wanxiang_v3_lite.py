...
LITE ADAPTATION (build_zh_wanxiang_v3_lite.py): discovers the lite tables in
Wanxiang's rolling lite-dicts.zip. The only functional change vs the v3
wrapper is table_name(), which strips the ".lite" infix from filenames like
"zi.lite.dict.yaml" so PREFERRED_TABLES / CORE_TABLES / EXCLUDED_TABLES keep
matching. wuzhong/renming are absent from the current lite-dicts.zip asset
and are skipped gracefully when missing.
...
def table_name(path: Path) -> str:
    # CHANGE (lite): lite-dicts.zip ships "zi.lite.dict.yaml"; strip the
    # ".lite" infix so names match PREFERRED_TABLES / CORE_TABLES /
    # EXCLUDED_TABLES below.
    name = path.name.removesuffix(".dict.yaml")
    return name.removesuffix(".lite")
...
def locate(root: Path) -> list[tuple[str, Path]]:
    ...
    for path in root.rglob("*.dict.yaml"):
        ...
        if name in EXCLUDED_TABLES:
            continue
        if name in by_name:
            raise SystemExit(f"Duplicate {name}.dict.yaml under {root}; found {path} and {by_name[name]}")
        by_name[name] = path
...
def main() -> None:
    ...
    # CHANGE (lite): the lite source yields ~3.85M readings / ~2.14M words
    # (2026-09), far above the v3 defaults (400k/600k). The compact step
    # re-ranks by max candidate probability and reaches deep into the v2
    # order, so a tight intermediate cap silently distorts its selection.
    # Keep ~20% headroom over the measured totals.
    p.add_argument("--lexicon-limit", type=int, default=2600000)
    p.add_argument("--max-ime-readings", type=int, default=4500000)
    ...
    delegate = Path(__file__).with_name("build_zh_wanxiang_v2_lite.py")  # CHANGE (lite)
